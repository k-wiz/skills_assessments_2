...
def without_duplicates(words):
    ...
    de_duped_words = list(set(words))

    return de_duped_words
...
def find_unique_common_items(items1, items2):
    ...
    **IMPORTANT**: you may not use `'if ___ in ___``
    ...
    unique_common_items = list(items1 & items2)
...
def translate_to_pirate_talk(phrase):
    ...
    for word in phrase:
        ...
        if word in translate_to_pirate_dict:
            pirate_word = translate_to_pirate_dict[word]
            pirate_speak.append(pirate_word)
        else:
            pirate_speak.append(word)

    pirate_speak = " ".join(pirate_speak)
...
def sort_by_word_length(words):
    ...
    for word in sorted_list:
        ...
        if length in length_dict:
            length_dict[length] = length_dict[length] + [word]
        else:
...
def kids_game(names):
    ...
    """NOTE: NOT FINISHED. RAN OUT OF TIME."""

    results = [names[0]]

    last_letter = results[len(results) - 1][len(names[0]) - 1]
    print last_letter
    ...
    for name in names:
        if name not in results:
            ...
            if name[0] == last_letter:
                ...
                print "continue"
                continue
            #if the name starts with last_letter, add name to list 
            #else break
